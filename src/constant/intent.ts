...
export const INTENT_LOCK_CKB_CELL_CAPACITY_FOR_CREATE_POOL = (
  ...
  return (
    BigInt(405_0000_0000) -
    (MINI_LOCK_LENGTH - BigInt(lockCapacity) * CKB_UNIT) / BigInt(2)
  );
};

export const INTENT_LOCK_UDT_CELL_CAPACITY_FOR_CREATE_POOL = (
  fromLock: CKBComponents.Script
) => {
  const lockCapacity =
    hexToBytes(fromLock.args).length + hexToBytes(fromLock.codeHash).length + 1;
...
export const CREATE_POOL_INTENT_ARGS_BUFFER_LENGTH =
  56 + 1 + 1 + 32 + 32 + 16 + 16;
